...
const TemporaryRedirectResponse = <T>(res: Response, message: string | null, data?: T) => {
    const response = { status: 307, success: true, data: data, message: message };
    if (res) {
        return res.status(307).json(response);
    } else {
        return response;
    }
}
